...
interface UseSmartDynamicFiltersReturn {
...
export function useSmartDynamicFilters(
  allDominios: Dominio[],
  allObjetivos: OGG[],
  allHerramientas: Herramienta[],
  filters: DynamicFilters
): UseSmartDynamicFiltersReturn {
  ...
  // Función para actualizar solo un tipo de filtro
  const updateFilterType = useCallback(async (filterType: 'dominios' | 'objetivos' | 'herramientas') => {
    ...
    try {
      ...
      const endpoint = `/api/cobit/${filterType}-filtrados`;
      ...
      const result = data.success ? data[filterType] : data;
  ...
  useEffect(() => {
    ...
    }, 200); // Debounce más corto para mejor responsividad
  ...
  }, [filters, getFiltersToUpdate, updateFilterType]);
